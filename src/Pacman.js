...
const createPacman = (x, y, tileSize, velocity, tileMap) => {
  ...
  function draw(ctx, pause, enemies, tileMap) {
    if (!pause) {
      move();
      animate();
    }
    eatDot();
    eatGhost(enemies, tileMap);
  ...
  function animate() {
    // console.log(currentMovingDirection, pacmanAnimationTimer);

    if (pacmanAnimationTimer == null) {
      return;
    }
  ...
  function eatGhost(enemies, tileMap) {
    if (tileMap.getPowerDotActive()) {
      for (let idx = enemies.length - 1; idx >= 0; idx--) {
        if (enemies[idx].collideWithCoordinate(x, y)) {
